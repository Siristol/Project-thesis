...
# Save model architecture and weights separately to avoid compilation issues
model_json = model.to_json()
with open(os.path.join(path_wd, model_name + '.json'), 'w') as json_file:
    json_file.write(model_json)
# Save weights with Keras 3.x format, then copy for SNN Toolbox compatibility
weights_path_keras = os.path.join(path_wd, model_name + '.weights.h5')
weights_path_snn = os.path.join(path_wd, model_name + '.h5')
model.save_weights(weights_path_keras)
# Copy weights file to the name SNN Toolbox expects
import shutil
shutil.copy2(weights_path_keras, weights_path_snn)
